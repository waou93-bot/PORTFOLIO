...
base.OUTPUT = ROOT / "hand-release-registered-v05.gif"


def clean_plate(image: Image.Image, _removal_mask: Image.Image):
    """Use a broad cleanup mask but tile only texture regions without symbols."""
    width, height = image.size
    ground = base.tiled_texture(image, (0, 0, 260, 145), image.size)
    # This crop is the left part of the slab, away from the engraved symbol;
    # v04's crop included the symbol and repeated it across the plate.
    slab = base.tiled_texture(image, (300, 145, 500, height), image.size)
