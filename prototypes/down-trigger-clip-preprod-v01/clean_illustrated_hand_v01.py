...
SOURCE = ROOT / "assets" / "derived-down-trigger" / "down-trigger-illustrated-hand-grip-provisional-v01.png"
TARGET = ROOT / "assets" / "derived-down-trigger" / "down-trigger-illustrated-hand-grip-provisional-v01-clean.png"
    ...
    for y in range(h):
        for x in range(w):
            ...
            if len(points) >= 600:
                for px, py in points:
                    keep[py, px] = True
    alpha = Image.fromarray((keep * 255).astype(np.uint8), mode="L").filter(ImageFilter.GaussianBlur(0.55))
