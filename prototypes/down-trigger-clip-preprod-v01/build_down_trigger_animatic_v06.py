...
HAND = OUT / "hand-release-illustrated-registered-v03.gif"
...
def close_transition(frame, index):
    # A brief paper-burn cut keeps the change from wide shot to macro intentional.
    image = frame.copy().convert("RGBA")
    overlay = Image.new("RGBA", (W, H), (184, 83, 57, 0))
    alpha = int(45 + index * 35)
...
def main():
    ...
    core_frames, core_durations = read_frames(CORE)
    hand_frames, hand_durations = read_frames(HAND)
    logo = Image.open(LOGO).convert("RGBA")

    frames = [opener(0, logo)]
    durations = [500]
    # Resistance and dizziness: the first part of the core pass.
    frames.extend(core_frames[:8])
    durations.extend([110] * 8)
    # Deliberate cut into the hand: preserve a small visual bridge.
    frames.append(close_transition(core_frames[7], 0))
    durations.append(90)
    frames.extend(hand_frames)
    durations.extend([120] * len(hand_frames))
    # Return to the wider body shot for the voluntary fall into the core.
    ...
    gif = OUT / "down-trigger-clip-animatic-v06.gif"
    ...
    sheet.save(OUT / "down-trigger-clip-animatic-v06-contact-sheet.jpg", quality=92)
    print(gif)
