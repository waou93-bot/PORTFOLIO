...
SOURCE = ROOT / "hand-release-test-v01.png"
OUTPUT = ROOT / "hand-release-test-v01.gif"
FRAME_SIZE = (720, 405)
FPS = 8
...
def crop_rows(image: Image.Image):
    ...
    if (width, height) != (1672, 941):
        raise ValueError(f"Unexpected hand sheet size: {(width, height)}")
    # Exact black separators measured on the generated three-row board.
    boxes = [(3, 3, 1669, 311), (3, 317, 1669, 624), (3, 631, 1669, 938)]
...
def exposure(keyframe: Image.Image, rng: random.Random, index: int) -> Image.Image:
    frame = keyframe.rotate(rng.uniform(-0.25, 0.25), resample=Image.Resampling.BICUBIC, fillcolor=(7, 8, 10))
    ...
    if index == 0:
        draw.ellipse((625, 50, 627, 52), fill=(210, 185, 154, 28))
    return shifted
